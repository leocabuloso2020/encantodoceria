...
import { Request, Response } from 'express'; // Tipagem para ambiente Node.js/Express
...
export default async function handler(req: Request, res: Response) {
  ...
  try {
    // Acessando variáveis de ambiente para Vercel (process.env)
    ...
    if (!MERCADO_PAGO_WEBHOOK_SECRET || !MERCADO_PAGO_ACCESS_TOKEN || !SUPABASE_URL || !SUPABASE_ANON_KEY) {
      console.error('Erro: Variáveis de ambiente do Mercado Pago ou Supabase não configuradas na Vercel.');
      return res.status(500).json({ error: 'Server configuration error' });
    }

    const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);

    const rawRequestBody = JSON.stringify(req.body); // req.body já é o objeto parseado pela Vercel
